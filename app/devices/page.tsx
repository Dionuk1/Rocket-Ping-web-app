...
function mapScannedToUi(scanned: ScannedDeviceResponse[]): UiDevice[] {
  if (scanned.length === 0) {
    return [];
  }
  ...
  return scanned.map((item, index) => {
    ...
    return {
      ...
      icon,
      statusText: item.online ? (item.latencyMs != null ? `Active - ${item.latencyMs} ms` : "Active") : "Offline",
      online: item.online,
    };
  });
}
...
export default function DevicesPage() {
  ...
  const loadLiveDevices = useCallback(
    async (showLoading: boolean) => {
      ...
      try {
        ...
        const message = error instanceof Error ? error.message : "Failed to scan local devices.";
  ...
  const uiDevices = useMemo(() => {
    const mapped = mapScannedToUi(liveDevices);
    if (!settings.showOfflineDevices) return mapped.filter((device) => device.online);
    return mapped;
  }, [liveDevices, settings.showOfflineDevices]);

  const statusText = useMemo(() => {
    if (loading) return "Scanning ARP entries...";
    if (loadError) return loadError;
    if (!lastUpdatedAt) return "No scan yet.";
    const ageSeconds = Math.max(0, Math.floor((now - lastUpdatedAt) / 1000));
    return ageSeconds <= 5 ? "Live" : `Last updated ${ageSeconds}s ago`;
  }, [lastUpdatedAt, loadError, loading, now]);
  ...
  return (
    <main className="space-y-4 pb-4 md:space-y-6 md:pb-8">
      <header className="flex items-center justify-between pt-2">
        <h1 className="text-[44px] font-bold text-white sm:text-[36px]">Connected Devices</h1>
        <AnimatedButton
      ...
      <Card className="border-[color:var(--np-border)] bg-[color:var(--np-card)] p-5 md:max-w-md">
        <div className="flex items-center justify-between">
          <div>
            <p className="text-[30px] text-white/50 sm:text-[22px]">Total Devices</p>
      ...
      <div className="space-y-3 md:grid md:grid-cols-2 md:gap-4 md:space-y-0">
        {!loading && uiDevices.length === 0 && (
          <Card className="border-[color:var(--np-border)] bg-[color:var(--np-card)] p-5 md:col-span-2">
            <p className="text-sm text-white/60">{loadError ? `Scan error: ${loadError}` : "No scanned devices found yet."}</p>
          </Card>
        )}
              ...
              <Card className="border-[color:var(--np-border)] bg-[color:var(--np-card)] p-5 transition hover:border-[color:var(--np-primary)]">
                ...
                <div className="mt-4 border-t border-white/10 pt-4">
                  <div className="grid grid-cols-[1fr_auto] gap-y-2 text-sm sm:text-base">
                    <p className="text-white/50">IP Address</p>
                    <p className="font-medium text-white">{device.ip}</p>
                    <p className="text-white/50">MAC Address</p>
                    <p className="font-medium text-white">{device.mac}</p>
                    <p className="text-white/50">Vendor</p>
                    <p className="font-medium text-white">{device.vendor || "Unknown"}</p>
                    <p className="text-white/50">OS Guess</p>
                    <p className="font-medium text-white">{device.osGuess || "Unknown"}</p>
                    <p className="text-white/50">Open Ports</p>
        ...
        <div className="fixed inset-0 z-50 flex items-end justify-center bg-black/45 p-4 md:items-center">
          <Card className="w-full max-w-xl border-[color:var(--np-border)] bg-[color:var(--np-card)] p-5">
            <div className="mb-3 flex items-center justify-between">
              <h2 className="text-xl font-semibold text-white">Device Detail</h2>
            ...
            <p className="text-sm text-white/70">Vendor: {selectedDevice.vendor || "Unknown"} | OS: {selectedDevice.osGuess || "Unknown"}</p>
            <div className="mt-4">
              <AnimatedButton
            ...
            <div className="mt-4">
              {detailLoading && <p className="text-sm text-white/60">Scanning common ports...</p>}
                  ...
                  <div className="max-h-48 overflow-auto rounded-xl border border-white/10 bg-white/5 p-2 text-xs">
                    {portsDetail.ports.map((port) => (
                      <p key={port.port} className={port.open ? "text-[color:var(--np-warn)]" : "text-white/40"}>
                        {port.port} ({port.service}) - {port.open ? "OPEN" : "closed"}
                      </p>
                    ))}
                  </div>
                </div>
