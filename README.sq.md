...
Kërkojnë token (`X-ROCKETPING-TOKEN`):
- `GET /network/info`
- `GET /scan/devices`
...
RocketPing përdor local agent për të ekzekutuar operacione të rrjetit/sistemit në Windows në mënyrë më të sigurt dhe më të qëndrueshme, krahasuar me ekzekutimin direkt të komandave në web routes.
...
- Vendose `speedtest.exe` në një nga path-et e mbështetura ose vendos `ROCKETPING_SPEEDTEST_BIN`.

### Probleme me auth të agent-it
- Verifiko që `ROCKETPING_TOKEN` në web app dhe local agent të jetë i njëjtë.
...
Kontributet janë të mirëpritura! Nëse doni ta përmirësoni RocketPing:
